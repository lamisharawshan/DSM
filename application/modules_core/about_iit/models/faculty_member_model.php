<?php
	...
	class faculty_member_model extends CI_Model
	{
	  ...
	  {
		$query = $this->db->query('SELECT * FROM `faculty_member` where mem_type="faculty" ORDER BY `ID` DESC');
		return $query->result();
	  }
	  function parttime_get_members()
	  {
	  		$query = $this->db->query('SELECT * FROM `faculty_member` where mem_type="Part time" ORDER BY `ID` DESC');
	  		return $query->result();
	  }
      function staff_get_members()
      {
            $query = $this->db->query('SELECT * FROM `faculty_member` where mem_type="Stuff" ORDER BY `ID` DESC');
	  		return $query->result();
      }
